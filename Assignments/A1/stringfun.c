...
// The purpose of these prototypes are for the compiler to know these functions/methods exist, their names, types &
// parameter types. Similar to initializing a variable in Java.
void  usage(char *);
...
int count_words(char *str){
    ...
    // Determine start of new word or end of word
    if(!word_start && currentChar != SPACE_CHAR){
    wc++;
    word_start = true;

    } else if (word_start && currentChar == SPACE_CHAR) {
    // End of word
    word_start = false;
    }
    }
...
void  word_print(char *str){
    ...
    if(currentChar == SPACE_CHAR || i == last_char_idx){
      printf(" (%d)\n", wlen);
      word_start = false;
    }
   }
...
int main(int argc, char *argv[]){
    ...
    switch (opt){
	    ...
	    // No need to call variable since it is being changed in place!
	    ...
	    // I added my own comment saying this before reading TODO4 haha. 
	    // This happens because in C, Strings are passed as  a pointer to functions!
	    // This allows the function to directly modify the memory of the original string,
	    // but if this was an issue in any case we could clone the array and work on the clone.
            break;
        case 'w':
            printf("Word Print\n----------\n");
        ...
        // I think the use for the default option here is "error handling" so the user
	// doesn't have any unusual experiences with the program when it's provided with invalid
	// input. I'm mainly a web developer but this is similar to having a '*' catch all 
	// unused routes and redirect a user to a 404 page or homepage.
        default:
            usage(argv[0]);
    ...
    /* The 'break' is keyword separating the various switch options. Without these breaks,
       the program would run all the options below the case. for example, if u had no breaks
       and the case was c, it would run ALL cases. Then the default option is the case where none
       of the cases provided are true. So when these args aren't identified it runs default case.
    */
}
